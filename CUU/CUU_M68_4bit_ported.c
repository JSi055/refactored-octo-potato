...
void CUU_M68_4_init(CUU_Interface *inter) {
    ...
    inter->write(0x33, 0);      // 8-bit function set, 8-bit function set
    inter->write(0x32, 0);      // 8-bit function set, 4-bit function set
}

void CUU_M68_4_write(uint8_t data, char rs) {    
        SETPIN(D4, data & 0x10);
        SETPIN(D5, data & 0x20);
        SETPIN(D6, data & 0x40);
        SETPIN(D7, data & 0x80);
...
uint8_t CUU_M68_4_read(bool rs) {
        DIRECTION(D4, 0);
        DIRECTION(D5, 0);
        DIRECTION(D6, 0);
        DIRECTION(D7, 0);
